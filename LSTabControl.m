...
- (id)initWithItem:(LSTabItem *)item {
    ...
    if (self) {
        ...
        button.titleLabel.font = [UIFont fontWithName:@"MicrosoftYaHei" size:20];
...
- (void)layoutSubviews { 
    ...
    if (CGRectIsEmpty(self.frame) == NO && CGRectIsEmpty(badgeView.frame) == NO) {
        badgeView.frame = CGRectMake(self.viewWidth - (badgeView.viewWidth/2) + 100.0f, -(badgeView.viewHeight/2),
